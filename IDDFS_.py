# created by chandima
# This to visualize the Enemy , Hero , kids and walls with IDDFS algorithm
...
#          This may be effect DFS limited and IDDFS's  time complexity and optimality
...
Max_Depth_Limit = 100  # maximum depth limit
...
ISMOVE = True  # to enable movement on Enemy character
...
pygame.display.set_caption("IDDFS Algorithm")
...
class Objects(object):
    ...
    def get_Neighbors(self):
        ...
        # for always prioritize  left most node
...
def Algo_DLS():
    ...
    global Total_Open_list_len
    global Total_Closed_list_len
    Depth_Limit = 1
    ...
    while not done:
        if is_rest:
            Total_Open_list_len += Total_Open_list_len + len(Open_list)
            Total_Closed_list_len += Total_Closed_list_len + len(Closed_list)
            ...
            for neighbor_ in neighbor_list:
                if neighbor_ is not None:
                    ...
                    if Valid_Node(neighbor_[0], neighbor_[1]):
                        ...
                        search_node.search_Depth = current_Node.search_Depth + 1

                        if search_node.search_Depth <= Depth_Limit or done:
                            search_node.draw()
                            Map.Grid[neighbor_[0]][neighbor_[1]].append(search_node)
                            Open_list.insert(0, search_node)
        ...
        if done:
            ...
            for path_node_ in path_node_list[1:]:
                ...
                print("node : {} {}".format(path_node_.Row, path_node_.Column))
            print("path: size  {}".format(len(path_node_list)))
        # end if State space is completed and max out Depth limit.
        if len(Open_list) < 1:
            if Depth_Limit < Max_Depth_Limit:
                ...
                for Row in range(MapSize):  # Drawing grid
                    for Column in range(MapSize):
                        if len(Map.Grid[Row][Column]) > 0:
                            if len(Map.Grid[Row][Column]) == 1:
                                if Map.Grid[Row][Column][0].Type == "Search":
                                    del Map.Grid[Row][Column][0]
                            else:
                                for obj_ in Map.Grid[Row][Column]:
                                    if obj_.Type == "Search":
                                        del obj_
...
Total_Open_list_len = 0
Total_Closed_list_len = 0
...
print("Final Open_list:{}".format(len(Open_list)))
print("Final closed_list:{}".format(len(Closed_list)))

print("Number of iterations: {}".format(Iteration_count))
# print("Total Open_list Length: {}".format(Total_Open_list_len))
print("Total Closed_list Length: {}".format(Total_Closed_list_len))
