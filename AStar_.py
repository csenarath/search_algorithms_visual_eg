# created by csenaraths
# This to visualize the Enemy , Hero , kids and walls with A* Search algorithm
...
#
#           at each search node he will calculate straight line distance (euclidean distance)(h(n))  to enemy at that time.
#           f(n)=g(n)+h(n) : g(n)-> traveled cost h(n) -> euclidean distance to enemy
...
SPEED_UP = 8  # to Speed Up #TileWidth = 30--> 8   TileWidth = 20--> 10
...
def Algo_AStar():
    ...
    while not done:
        ...
        for item_ in Open_list:
            ...
            #f(n)=h(n)+g(n)
            item_.f_cost = item_.Distance + item_.Traveled_Distance
        Open_list.sort(key=lambda x: (x.f_cost, x.Distance))
            ...
            for neighbor_ in neighbor_list:
                if neighbor_ is not None:
                    ...
                    if Valid_Node(neighbor_[0], neighbor_[1]):
                        ...
                        if Map.Hero.compare_coordination(neighbor_[0], neighbor_[1]):
                            search_node.Traveled_Distance = 0
                        else:
                            search_node.Traveled_Distance = current_Node.Traveled_Distance + 1

                        Map.Grid[neighbor_[0]][neighbor_[1]].append(search_node)
                        Open_list.append(search_node)
